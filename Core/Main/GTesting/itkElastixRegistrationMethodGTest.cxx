...
// Tests the origin of the output image after registering two images.
GTEST_TEST(itkElastixRegistrationMethod, OutputOrigin)
{
  constexpr auto ImageDimension = 2U;
  using ImageType = itk::Image<float, ImageDimension>;
  using SizeType = itk::Size<ImageDimension>;
  ...
  const OffsetType translationOffset{ { 1, -2 } };
  const auto       regionSize = SizeType::Filled(2);
  const SizeType   imageSize{ { 20, 28 } };
  const IndexType  fixedImageRegionIndex{ { 10, 12 } };
  ...
  for (const auto fixedImageOrigin : { MakePoint(-1.0, -2.0), ImageType::PointType(), MakePoint(0.25, 0.75) })
  {
    ...
    for (const auto movingImageOrigin : { MakePoint(-1.0, -2.0), ImageType::PointType(), MakePoint(0.25, 0.75) })
    {
      ...
      {
        EXPECT_EQ(std::round(transformParameters[i] + fixedImageOrigin[i] - movingImageOrigin[i]), translationOffset[i])
          << fixedImageOrigin << movingImageOrigin;
      }
    }
  }
}
